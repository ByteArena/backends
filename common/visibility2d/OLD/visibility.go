...
func getTrianglePoints(origin Point, angle1, angle2 float64, segment *Segment) [2]Point {
	...
	if segment != nil {
		p3.X = segment.PointA.X
		p3.Y = segment.PointA.Y
		p4.X = segment.PointB.X
		p4.Y = segment.PointB.Y
	} else {
					...
					if openSegment != nil {
						...
						output = append(output, Visible{
							...
							Complete: vector.MakeSegment2(
								vector.MakeVector2(openSegment.PointA.X, openSegment.PointA.Y),
								vector.MakeVector2(openSegment.PointB.X, openSegment.PointB.Y),
							),
