...
func (server *Server) Spawnagent(config protocol.Config) {
	...
	container, err := server.containerorchestrator.CreateAgentContainer(agent.GetId(), server.host, server.port, server.agentdir, config)
	if err != nil {
		log.Panicln(err)
	}
